...
class TemporalUnetImage(nn.Module):
    def __init__(
        self,
        horizon,
        transition_dim,
        ...
        for k, v in self.shape_meta["all_shapes"].items():
            if "rgb" in k:
                self.image_encoders[k] = instantiate(image_encoder_config)
...
class TemporalUnetImageTaskEmb(nn.Module):
    def __init__(
        self,
        horizon,
        transition_dim,
        ...
        for k, v in self.shape_meta["all_shapes"].items():
            if "rgb" in k:
                self.image_encoders[k] = instantiate(image_encoder_config)
        dims = [transition_dim + task_embedding_dim, *map(lambda m: dim * m, dim_mults)]
        in_out = list(zip(dims[:-1], dims[1:]))
        print(f'[ models/temporal ] Channel dimensions: {in_out}')
        ...
        self.final_conv = nn.Sequential(
            Conv1dBlock(dim, dim, kernel_size=5),
            nn.Conv1d(dim, transition_dim + task_embedding_dim, 1),
        )
